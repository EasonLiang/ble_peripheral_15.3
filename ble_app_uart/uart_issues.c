...
static void timers_init(void){    ACEE( app_timer_init() );      }

static void gap_params_init(void)   {
        ble_gap_conn_params_t   gap_conn_params = {0};
        ble_gap_conn_sec_mode_t sec_mode;
    BLE_GAP_CONN_SEC_MODE_SET_OPEN(&sec_mode);
    ACEE( sd_ble_gap_device_name_set(&sec_mode,  (const uint8_t *) DEVICE_NAME, strlen(DEVICE_NAME));
        gap_conn_params.min_conn_interval = MIN_CONN_INTERVAL;
        gap_conn_params.max_conn_interval = MAX_CONN_INTERVAL;
        gap_conn_params.slave_latency     = SLAVE_LATENCY;
        gap_conn_params.conn_sup_timeout  = CONN_SUP_TIMEOUT;
    ACEE( sd_ble_gap_ppcp_set(&gap_conn_params) );
}

static void nrf_qwr_error_handler(uint32_t nrf_error)       {    APP_ERROR_HANDLER(nrf_error);      }

/**@brief Function for handling the data from the Nordic UART Service.
 * @details This function will process the data received from the Nordic UART BLE Service and send it to the UART module.
...
static void services_init(void){
    ...
    ACEE( ble_nus_init(&m_nus, &nus_init) );
}
...
static void sleep_mode_enter(void)  {
    ACEE( bsp_indication_set(BSP_INDICATE_IDLE) );
    ACEE( bsp_btn_ble_sleep_mode_prepare() );
    ACEE (sd_power_system_off() );  // Go to system-off mode (this function will not return; wakeup will cause a reset).
}
...
static void ble_evt_handler(ble_evt_t const * p_ble_evt, void * p_context)
{
    switch (p_ble_evt->header.evt_id)    {
        case BLE_GAP_EVT_CONNECTED:
            ...
            ACEE( bsp_indication_set(BSP_INDICATE_CONNECTED) );
...
void gatt_init(void){
    ACEE( nrf_ble_gatt_init(&m_gatt, gatt_evt_handler) );
    ACEE( nrf_ble_gatt_att_mtu_periph_set(&m_gatt, NRF_SDH_BLE_GATT_MAX_MTU_SIZE) );
}
 ...
 * @details This function will receive a single character from the app_uart module and append it to a string. The string will be be sent over 
 * BLE when the last character received was a 'new line' '\n' (hex 0x0A) or if the string has reached the maximum data length. */
void uart_event_handle(app_uart_evt_t * p_event)
{
    static uint8_t data_array[BLE_NUS_MAX_DATA_LEN] , index = 0;
    uint32_t       err_code;
    ...
    static void on_adv_evt(ble_adv_evt_t ble_adv_evt){
        switch (ble_adv_evt)  {
            case BLE_ADV_EVT_FAST:
                ACEE( bsp_indication_set(BSP_INDICATE_ADVERTISING) );
                break;
...
static void advertising_init(void)
{
    ...
    ACEE( ble_advertising_init(&m_advertising, &init) );
    ble_advertising_conn_cfg_tag_set(&m_advertising, APP_BLE_CONN_CFG_TAG);
}

static void buttons_leds_init(bool * p_erase_bonds){
    bsp_event_t startup_event;
    ACEE( bsp_init(BSP_INIT_LEDS | BSP_INIT_BUTTONS, bsp_event_handler));
    ACEE( bsp_btn_ble_init(NULL, &startup_event) );
...
static void advertising_start(void)         {    ACEE( ble_advertising_start(&m_advertising, BLE_ADV_MODE_FAST) );  }

int main(void)
{
    bool erase_bonds;
    uart_init();    log_init();     timers_init();  buttons_leds_init(&erase_bonds);    power_management_init();
