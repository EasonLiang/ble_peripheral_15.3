 ...
 * (and also Battery and Device Information services). This application uses the srvlib_conn_params module. */
...
static void battery_level_update(void)      {
    ...
    if (    (err_code != NRF_SUCCESS) && (err_code != NRF_ERROR_INVALID_STATE) &&
        (err_code != NRF_ERROR_RESOURCES) && (err_code != NRF_ERROR_BUSY) && (err_code != BLE_ERROR_GATTS_SYS_ATTR_MISSING)
       )
...
static void read_glucose_measurement(void)  {
    ...
    rec.meas.flags = BLE_GLS_MEAS_FLAG_TIME_OFFSET | BLE_GLS_MEAS_FLAG_CONC_TYPE_LOC | BLE_GLS_MEAS_FLAG_UNITS_MOL_L;
    ...
    rec.meas.base_time.seconds              = s_secs;
